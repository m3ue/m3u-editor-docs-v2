...
- **VOD plays fine in the browser but fails/hangs from M3U-Editor, while sync and artwork work fine**: if M3U-Editor is running in a VPN-routed container (e.g. the [Gluetun VPN deployment](/docs/deployment/docker-compose#vpn-deployment)) on the **same host** as Plex, the streaming request can hairpin through Docker's own bridge network and reach Plex from a Docker-internal IP instead of your real LAN IP. Plex then applies its remote-stream bandwidth cap to what should be a local, unrestricted connection. Check Plex's log for a `Bandwidth exceeded` warning right after a failed playback attempt, and see the [VPN deployment troubleshooting section](/docs/deployment/docker-compose#troubleshooting-media-server-plexembyjellyfin-vod-playback-fails-but-sync-works) for the fix (add the Docker bridge subnet to Plex's **LAN Networks** setting).